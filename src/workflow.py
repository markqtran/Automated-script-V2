...
def run_full_workflow(
    ...
    *,
    ...
    # Step 3 — Premiere automation script
    jsx_path = write_premiere_setup_script(cfg, ssd_path, folder_name, prproj_path)
    console.print(f"\n[bold]Premiere script written:[/bold] {jsx_path}")

    if open_premiere:
        from .new_project import _open_premiere

        _open_premiere(cfg, prproj_path if prproj_path.exists() else None, ssd_path)
        console.print("\n[bold yellow]In Premiere (one-time per session):[/bold yellow]")
        console.print(f"  File > Scripts > Run Script File > {jsx_path.name}")
        console.print("  That creates the project, imports Video/, and opens Media Encoder when you create proxies.")
